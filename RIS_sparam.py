...
noDUT_meas  = rf.Network(pathMeas+'noDUT_1.s2p')
metal_meas  = rf.Network(pathMeas+'metal_1.s2p')
...
for i in range(len(voltages)):
    ...
    pathV = "{path}{voltage}_1.s2p".format(path=pathMeas, voltage=voltages[i])
    ...
    RAmp_meas_21.insert(i, (metal_meas_21_gated - V_meas_21[i]))
    #RAmp_meas_21_gated.insert(i, (metal_meas_21_gated - V_meas_21_gated[i]) / (metal_meas_21_gated - noDUT_meas_21_gated))
...
noDUT_meas_21.plot_z_mag()
metal_meas_21.plot_z_mag()
for i in range(len(voltages)):
    V_meas_21[i].plot_z_mag()
plt.ylabel('Z11 [dB]')
plt.xlabel('Frequency')
#plt.xlim([7.5e9, 12.5e9])
plt.tight_layout()
